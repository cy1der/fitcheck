...
describe('posts', () => {
  ...
  scenario('creates a post', async (scenario: StandardScenario) => {
    const result = await createPost({
      input: { id: 9289065, fitId: scenario.post.two.fitId },
    })

    expect(result.id).toEqual(9289065)
    expect(result.fitId).toEqual(scenario.post.two.fitId)
  })
  ...
  scenario('updates a post', async (scenario: StandardScenario) => {
    ...
    expect(result.id).toEqual(5605554)
  })
